...
def COMPAS_experiment():
    ...
    for train_index, test_index in kf1.split(COMPAS):
        ...
        for i in admissible_features + fair_features:
        ...
        results.append(
            ['COMPAS', 'FairExp-guaranteed', selected_names, fold, rod_FairExp_,
    ...
    #     X_train_repaired = train_repaired.loc[:, [i for i in list(COMPAS) if i != target]]
    #
    #     X_test_capuchin = COMPAS.iloc[
    #         test_index, [idx for idx, i in enumerate(list(COMPAS)) if i != target]]
    #     X_test_capuchin.reset_index(inplace=True, drop=True)
    #     y_test_capuchin = COMPAS.iloc[test_index, [idx for idx, i in enumerate(list(COMPAS))]]
    ...
    #
    #     results.append(
    #         ['Adult', 'Capuchin ad-hoc', list(COMPAS), fold, rod_capuchin,
