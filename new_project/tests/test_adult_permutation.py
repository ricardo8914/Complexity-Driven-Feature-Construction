...
for train_index, test_index in kf1.split(adult_df):
    ...
    print('Computing the permutation importances')

    rf = RandomForestClassifier(n_estimators=200, max_features=1.0, n_jobs=-1)

    rf.fit(transformed_train_2, np.ravel(y_train.to_numpy()))

    result = permutation_importance(rf, transformed_train_2, np.ravel(y_train.to_numpy()), n_repeats=10,
                                    random_state=42, n_jobs=-1)

    important = result.importances_mean[result.importances_mean > 0].shape[0]
    sorted_idx = result.importances_mean.argsort()[::-1][:important]
    best = [transformed_columns_complete[i] for i in sorted_idx]
    best_features_train = transformed_train_2[:, sorted_idx]
    best_features_test = transformed_test_2[:, sorted_idx]

    print('Starting to evaluate genetic feature selection')
    selected_array = evolution(transformed_train_2[:, sorted_idx], np.ravel(y_train.to_numpy()),
                               scorers=[rod, f1], cv_splitter=5,
                               max_search_time=60)
    ...
    for i in selected_array:
        ...
        genetic_trained.append(cv_grid_evolutionary.fit(best_features_train[:, i], np.ravel(y_train.to_numpy())))
    ...
    for idx, i in enumerate(tqdm(best)):
        if i != sensitive_feature:
            ############ TO-DO cross validate with train data to avoid overfitting fair-wise

            #selected_idx_causal.extend([sorted_idx[idx]])
            #selected_names_causal.extend([i])

            test_model = LogisticRegression()
            test_model.fit(transformed_train_2[:, [sorted_idx[idx]]], np.ravel(y_train.to_numpy()))

            proba_causal = test_model.predict_proba(transformed_train_2[:, [sorted_idx[idx]]])[:, 1]
            predictions_causal = test_model.predict(transformed_train_2[:, [sorted_idx[idx]]])
            ...
            selected_df_causal = pd.DataFrame(data=transformed_train_2[:, [sorted_idx[idx]]], columns=[i])
            ...
            if d_separation(test_df_causal, sensitive=sensitive_feature, target='outcome'):
                ...
                if ll_causal - logLoss_causal >= 0.001:
                    ...
                    selected_idx_causal.remove(sorted_idx[idx])
            else:
                selected_names_causal.remove(i)
                selected_idx_causal.remove(sorted_idx[idx])
    ...
    for idx, i in enumerate(genetic_trained):
        outcome_gen = i.predict(best_features_test[:, selected_array[idx]])
        y_pred_proba_gen = i.predict_proba(best_features_test[:, selected_array[idx]])[:, 1]
        ...
        representation = [best[i] for i in my_list]
...
summary_df.to_csv(path_or_buf=results_path + '/summary_adult_genetic_50it_permutation_complexity_4_causaljoin.csv', index=False)
