...
def scalability_experiment(sampling=1.0, complexity=4, number_model_parallelism=mp.cpu_count(), number_speculativ_parallelism=1, number_kfold_parallelism=1):
    ...
    for train_index, test_index in kf1.split(df):
        ...
        selected_features_ = repair_algorithm(X_train, names, train_df_e, y_train, sensitive_feature,
                                              ...
                                              my_model,
                                              sampling=sampling, number_of_paralllelism=number_speculativ_parallelism, kfold_parallelism=number_kfold_parallelism)
...
if __name__ == '__main__':
    ...
    german_results = scalability_experiment(number_model_parallelism=mp.cpu_count(), number_speculativ_parallelism=1,
                                       number_kfold_parallelism=1)
    german_results.to_csv(path_or_buf=home + '/Complexity-Driven-Feature-Construction/results/new_mpA,sp1,fp1.csv',
                          index=False)

    german_results = scalability_experiment(number_model_parallelism=1, number_speculativ_parallelism=1,
                                       number_kfold_parallelism=5)
    german_results.to_csv(path_or_buf=home + '/Complexity-Driven-Feature-Construction/results/new_mp1,sp1,fp5.csv',
                          index=False)

    german_results = scalability_experiment(number_model_parallelism=1, number_speculativ_parallelism=mp.cpu_count(),
                                       number_kfold_parallelism=1)
    german_results.to_csv(path_or_buf=home + '/Complexity-Driven-Feature-Construction/results/new_mp1,spA,fp1.csv',
                          index=False)

    german_results = scalability_experiment(number_model_parallelism=1, number_speculativ_parallelism=5,
                                       number_kfold_parallelism=5)
    german_results.to_csv(path_or_buf=home + '/Complexity-Driven-Feature-Construction/results/new_mp1,sp5,fp5.csv',
                          index=False)

    german_results = scalability_experiment(number_model_parallelism=mp.cpu_count(), number_speculativ_parallelism=5,
                                       number_kfold_parallelism=5)
    german_results.to_csv(path_or_buf=home + '/Complexity-Driven-Feature-Construction/results/new_mpA,sp5,fp5.csv',
                          index=False)