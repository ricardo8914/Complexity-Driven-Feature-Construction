...
for k,v in cost_2_raw_features.items():
    for candidate in v:
        ...
        if c in list(candidate.runtime_properties.keys()):
            print(str(candidate) + ": Test Score = "+ "{0:.2f}".format(candidate.runtime_properties['test_score'])
                  + ' ,Fair Test Score = ' + "{0:.2f}".format(candidate.runtime_properties['fair_test_score']) +
                  ', Area under curve FNR FPR= ' + "{0:.2f}".format(candidate.runtime_properties['AUC_FN_FP']))
