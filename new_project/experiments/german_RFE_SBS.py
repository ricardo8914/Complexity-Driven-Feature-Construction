...
for train_index, test_index in kf1.split(credit_df):
    ...
    for idx, i in enumerate(all_2_combinations):
        ...
        column_transformation = Pipeline([('new_construction',
                                           ConstructionTransformer(c_max=complexity, max_time_secs=1000000, scoring=f1,
                                                                   n_jobs=4,
        ...
        transformations2_generate = [t for t in all_transformations if ((t.get_name()).strip() not in unique_features
                                                                        and not (
                            t.get_complexity() == 2 and re.search(sensitive_feature, t.get_name().strip())))]
        transformations2_generate_idx = [idx for idx, t in enumerate(all_transformations) if
                                         (t.get_name()).strip() not in unique_features
                                         and not (t.get_complexity() == 2 and re.search(sensitive_feature,
                                                                                        t.get_name().strip()))]
        ...
        for idj, j in enumerate(mask):
            ...
            if np.isnan(transformed_train_i[:, [j]]).sum() == 0 and np.isinf(transformed_test_i[:, [j]]).sum() == 0 and \
                    np.unique(transformed_train_i[:, [j]]).shape[0] > 1 and np.unique(transformed_test_i[:, [j]]).shape[
                0] > 1:
    ...
    # sample_idx_all = np.random.randint(all_allowed_train.shape[0], size=round(all_allowed_train.shape[0]))
    ...
    selector = RFECV(estimator, step=0.001, cv=3, n_jobs=-1, scoring='f1', min_features_to_select=5)
    selector = selector.fit(all_allowed_train, np.ravel(y_train.to_numpy()))
    ...
    def eliminate(candidate):
        ...
        if accepted_features_remove not in u:
            ...
            transformed_train_remove = all_allowed_train[:, intersection]
            ...
            cv_scores.fit(transformed_train_remove, np.ravel(y_train.to_numpy()))
    ...
    print('selected representation train, Index=' + str(where_pareto[min_dist].item()) + ' ,F1=' + str(
        all_visited[where_pareto[min_dist].item()][2]) + ', ROD =' + str(all_visited[where_pareto[min_dist].item()][3]))
    # selected_indices = all_visited[where_pareto[min_dist].item()][4]
    ...
    runtimes.append(
        ['FC_SBFS', complexity, all_allowed_train.shape[0], all_allowed_train.shape[1], end_time, count + 1])
    ...
    visited_representations_train = visited_representations_train.loc[:,
                                    ['Representation', 'Size', 'F1', 'ROD', 'Fold']]

    if CF:
        visited_representations_train.to_csv(
            path_or_buf=results_path + '/credit_df_FC_SBFS_visited_representations_train_complexity_' + str(complexity)
                        + '_CF_' + str(count + 1) + '.csv', index=False)
    else:
        visited_representations_train.to_csv(
            path_or_buf=results_path + '/credit_df_SBFS_visited_representations_train_complexity_' + str(
...
if CF:
    summary_df.to_csv(path_or_buf=results_path + '/credit_df_FC_SBFS_complexity_' + str(complexity) + '_CF' + '.csv',
                      index=False)
    runtimes_df.to_csv(
        path_or_buf=results_path + '/runtimes_credit_df_FC_SBFS_complexity_' + str(complexity) + '_CF' + '.csv',
        index=False)
else:
    summary_df.to_csv(path_or_buf=results_path + '/credit_df_FC_SBFS_complexity_' + str(complexity) + '.csv',
                      index=False)
    runtimes_df.to_csv(
        path_or_buf=results_path + '/runtimes_credit_df_FC_SBFS_complexity_' + str(complexity) + '.csv',
        index=False)
