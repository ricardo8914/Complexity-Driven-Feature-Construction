...
df = pd.read_csv(path + '/Traffic_Violations.csv', sep=',', header=0)
df.dropna(inplace=True)

df.drop(columns=['Description', 'Gender', 'SeqID', 'Article', 'Charge', 'Search Reason For Stop',
                 'Search Conducted', 'HAZMAT', 'Model', 'Make', 'Latitude', 'Longitude', 'Location',
...
for s in [50000]:
    sample_idx = np.random.randint(df.shape[0], size=s)
    sample_df = df.iloc[sample_idx]
    sample_df.reset_index(drop=True, inplace=True)

    kf1 = KFold(n_splits=3, random_state=42, shuffle=True)
    for train_index, test_index in kf1.split(sample_df):
        ...
        train_df = sample_df.iloc[train_index]
        test_df = sample_df.iloc[test_index]
        ...
        for idx, i in enumerate(all_2_combinations):
            ...
            for x in features2_build:
                if sample_df[x].dtype != np.dtype('O'):
            ...
            for idj, j in enumerate(mask):
                ...
                if np.isnan(transformed_train_i[:, [j]]).sum() == 0 and np.isnan(transformed_test_i[:, [j]]).sum() == 0 and np.isinf(transformed_test_i[:, [j]]).sum() == 0 and \
                        np.unique(transformed_train_i[:, [j]]).shape[0] > 1 and np.isinf(transformed_train_i[:, [j]]).sum() == 0 and np.unique(transformed_test_i[:, [j]]).shape[0] > 1:
        ...
        method_list.append(['FC-FS-BS', selected_representation[3], selected_representation[2], selected_representation[0],
        ...
        runtimes.extend([count_transformations, time_2_create_transformations, filtered_transformations, time_2_CF, time_2_FR, time_2_SR,
                         time_2_create_transformations+time_2_CF+time_2_FR+time_2_SR, count +1, s])
        ...
        if CF:
            visited_representations_train.to_csv(path_or_buf=results_path + '/' + dataset +'_complete_visited_representations_train_complexity_' + str(complexity)
                                                       + '_CF_' + str(count+1) + '_' + str(s) + '.csv', index=False)
            ...
            visited_representations_train.to_csv(
                path_or_buf=results_path + '/' + dataset + '_complete_visited_representations_train_complexity_' + str(complexity) + '_' + str(count + 1) + '_' + str(s) + '.csv',
                index=False)
            visited_representations_test.to_csv(
                path_or_buf=results_path + '/' + dataset + '_complete_visited_representations_test_complexity_' + str(
                    complexity) + '_' + str(count + 1) + '_' + str(s) + '.csv',
                index=False)
    ...
    runtimes_df = pd.DataFrame(runtimes_list, columns=['Combinations', 'Complexity', 'Rows', 'Transformations',
                                                      ...
                                                      'Time_2_CF', 'Time_2_FR', 'Time_2_SR', 'Total_runtime_SFFS_BF', 'Fold', 'Sample Size'])
    ...
    if CF:
        summary_df.to_csv(path_or_buf=results_path + '/complete_' + dataset +'_results_complexity_' + str(complexity) + '_' + str(s) + '_CF.csv', index=False)
        runtimes_df.to_csv(
            path_or_buf=results_path + '/' + dataset + '_runtimes_complexity_' + str(complexity) + '_CF_' + str(count) + '_' + str(s) + '.csv',
            index=False)
    else:
        summary_df.to_csv(path_or_buf=results_path + '/complete_' + dataset + '_results_complexity_' + str(complexity) + '_' + str(s) + '.csv', index=False)
        runtimes_df.to_csv(path_or_buf=results_path + '/' + dataset + '_runtimes_complexity_' + str(complexity) + '_' + str(s) + '.csv',
                           index=False)
