...
for train_index, test_index in kf1.split(credit_df):
    ...
    for idg, g in enumerate(selected_array):
        if all_allowed_train[:, g].shape[1] > 0:
            ...
            rod_genetic_train = ROD.ROD(y_pred=predicted_genetic_proba_train, sensitive=X_train.loc[:, [sensitive_feature]],
                                  admissible=X_train.loc[:, admissible_features],
                                  protected='young', name='genetic_credit_df')
...
if CF:
    summary_df.to_csv(path_or_buf=results_path + '/credit_df_genetic_complexity_' + str(complexity) + '_CF' + '.csv', index=False)
    runtimes_df.to_csv(path_or_buf=results_path + '/runtimes_credit_df_genetic_complexity_' + str(complexity) + '_CF' + '.csv',
                      index=False)
    visited_train_df.to_csv(path_or_buf=results_path + '/visited_train_credit_df_genetic_complexity_' + str(complexity) + '_CF' + '.csv',
                      index=False)
    visited_test_df.to_csv(
        path_or_buf=results_path + '/visited_test_credit_df_genetic_complexity_' + str(complexity) + '_CF' + '.csv',
        index=False)
else:
    summary_df.to_csv(path_or_buf=results_path + '/credit_df_genetic_complexity_' + str(complexity) + '.csv', index=False)
    runtimes_df.to_csv(path_or_buf=results_path + '/runtimes_credit_df_genetic_complexity_' + str(complexity) + '.csv', index=False)
    visited_train_df.to_csv(
        path_or_buf=results_path + '/visited_train_credit_df_genetic_complexity_' + str(complexity) + '.csv',
        index=False)
    visited_test_df.to_csv(
        path_or_buf=results_path + '/visited_test_credit_df_genetic_complexity_' + str(complexity) + '.csv',
        index=False)