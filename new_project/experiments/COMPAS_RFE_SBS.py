...
for train_index, test_index in kf1.split(COMPAS):
    ...
    if CF:
        visited_representations_train.to_csv(
            path_or_buf=results_path + '/COMPAS_FC_SBFS_visited_representations_train_complexity_' + str(complexity)
                        + '_CF_' + str(count + 1) + '.csv', index=False)
    else:
        visited_representations_train.to_csv(
            path_or_buf=results_path + '/COMPAS_SBFS_visited_representations_train_complexity_' + str(
...
if CF:
    summary_df.to_csv(path_or_buf=results_path + '/COMPAS_FC_SBFS_complexity_' + str(complexity) + '_CF' + '.csv',
                      index=False)
    runtimes_df.to_csv(
        path_or_buf=results_path + '/runtimes_COMPAS_FC_SBFS_complexity_' + str(complexity) + '_CF' + '.csv',
        index=False)
else:
    summary_df.to_csv(path_or_buf=results_path + '/COMPAS_FC_SBFS_complexity_' + str(complexity) + '.csv',
                      index=False)
    runtimes_df.to_csv(
        path_or_buf=results_path + '/runtimes_COMPAS_FC_SBFS_complexity_' + str(complexity) + '.csv',
        index=False)
