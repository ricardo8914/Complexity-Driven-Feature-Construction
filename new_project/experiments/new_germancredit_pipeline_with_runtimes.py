...
for train_index, test_index in kf1.split(credit_df):
    ...
    for idx, i in enumerate(all_2_combinations):
        ...
        for x in features2_build:
            if credit_df[x].dtype != np.dtype('O'):
        ...
        if np.isnan(transformed_train_i).sum() == 0 and np.isinf(transformed_test_i).sum() == 0:
            ...
            for idj, j in enumerate(mask):
                ...
                if np.unique(transformed_train_i[:, [j]]).shape[0] > 1 and np.unique(transformed_test_i[:, [j]]).shape[0] > 1:
                    ...
                    rod_ff = ROD.ROD(y_pred=predicted_ff_proba, sensitive=X_test.loc[:, [sensitive_feature]],
                                     ...
                                     protected='young', name='forward_credit_df')
    ...
    #                               admissible=X_test.loc[:, admissible_features],
    #                               protected='young', name='genetic_credit_df')
    ...
    if CF:
        visited_representations_train.to_csv(path_or_buf=results_path + '/credit_df_complete_visited_representations_train_complexity_' + str(complexity)
                                                   + '_CF_' + str(count+1) + '.csv', index=False)
        visited_representations_test.to_csv(
            path_or_buf=results_path + '/credit_df_complete_visited_representations_test_complexity_' + str(complexity)
                        + '_CF_' + str(count + 1) + '.csv', index=False)
    else:
        visited_representations_train.to_csv(
            path_or_buf=results_path + '/credit_df_complete_visited_representations_train_complexity_' + str(complexity) + '_' + str(count + 1) + '.csv',
            index=False)
        visited_representations_test.to_csv(
            path_or_buf=results_path + '/credit_df_complete_visited_representations_test_complexity_' + str(
    ...
    credit_df_dropped = credit_df.copy()
    dropped_columns = list(credit_df_dropped)
    ...
    if len(inadmissible_features) > 0:
        X_train_dropped = X_train.drop(columns=[sensitive_feature, inadmissible_features])
        X_test_dropped = X_test.drop(columns=[sensitive_feature, inadmissible_features])
    else:
    ...
    rod_dropped = ROD.ROD(y_pred=predicted_dropped_proba, sensitive=X_test.loc[:, [sensitive_feature]],
                          admissible=X_test.loc[:, admissible_features],
                          protected='young', name='dropped_credit_df')
    ...
    rod_original = ROD.ROD(y_pred=predicted_original_proba, sensitive=X_test.loc[:, [sensitive_feature]],
                           admissible=X_test.loc[:, admissible_features],
                           protected='young', name='original_credit_df')
    ...
    if CF:
        method_df.to_csv(
            path_or_buf=results_path + '/complete_credit_df_results_complexity_' + str(complexity) + '_CF_' + str(count+1) + '.csv', index=False)
    else:
        method_df.to_csv(path_or_buf=results_path + '/complete_credit_df_results_complexity_' + str(complexity) + '_' + str(count+1) + '.csv',
                          index=False)
    ...
    if CF:
        runtimes_df.to_csv(path_or_buf=results_path + '/credit_df_runtimes_complexity_' + str(complexity) + '_CF_' + str(count) + '.csv',
                       index=False)
    else:
        runtimes_df.to_csv(path_or_buf=results_path + '/credit_df_runtimes_complexity_' + str(complexity) + '_' + str(count) + '.csv',
                           index=False)
...
if CF:
    summary_df.to_csv(path_or_buf=results_path + '/complete_credit_df_results_complexity_' + str(complexity) + '_CF.csv', index=False)
else:
    summary_df.to_csv(path_or_buf=results_path + '/complete_credit_df_results_complexity_' + str(complexity) + '.csv', index=False)