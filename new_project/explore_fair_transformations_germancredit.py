...
for i in all_2_combinations:
    ...
    column_transformation = Pipeline([('new_construction',
                                       ConstructionTransformer(c_max=5, max_time_secs=1000000, scoring=f1, n_jobs=7,
    ...
    X = credit_df.loc[:, all_features]
    y = credit_df.loc[:, target].to_numpy()
    ...
    def generate_instance(candidate, root=i):
        ...
        if len(candidate.parents) >= 1:
            ...
            for p in candidate.parents:
                ...
                if (p.get_name()).strip() == sensitive_feature:
                    ...
                    if np.isnan(transformed_test_p).sum() == 0 and np.isinf(transformed_test_p).sum() == 0:
        ...
        if j != sensitive_feature:
            ...
            if np.isnan(transformed_test_c).sum() == 0 and np.isinf(transformed_test_c).sum() == 0:
...
instances_df.to_csv(path_or_buf=results_path + '/feature_analysis_german_credit.csv', index=False)
