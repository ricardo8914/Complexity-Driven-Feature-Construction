...
from fairexp_optimistic_multiple import extend_dataframe_complete, repair_algorithm
import numpy as np
...
adult.drop(columns=['fnlwgt', 'relationship', 'native-country'], inplace=True)
...
adult['race'] = adult['race'].apply(lambda x: 'Other' if x != 'White' else 'White')

target = 'income'
sensitive_feature = 'sex'
inadmissible_feature = 'marital-status'
protected = ['Female', 'Other']
protected_capuchin = ' Female'
sensitive_features = ['sex', 'race']
admissible_features = [i for i in list(adult) if i not in sensitive_features and i != inadmissible_feature and i != target]
...
def adult_experiment():
    ...
    for train_index, test_index in kf1.split(adult):
        ...
        j = selected_names + admissible_features
        ...
        rod_scores = []
        for s, p in list(zip(sensitive_features, protected)):

            rod_scores.append(ROD.ROD(y_pred=y_pred_proba, df=test_df_e,
                                   sensitive=s,
                                   admissible=admissible_features, protected=p, mb=mb))
        ...
        print('ROD FairExp: ' + str(rod_scores))
        ...
        results.append(
            ['Adult', 'FairExp', selected_names, fold, *rod_scores,
        ...
        train_df = adult.iloc[train_index]
        test_df = adult.iloc[test_index]
        train_df.reset_index(drop=True, inplace=True)
        test_df.reset_index(drop=True, inplace=True)

        X_train = adult.iloc[train_index, [list(adult).index(i) for i in list(adult) if i != target]]
        X_test = adult.iloc[test_index, [list(adult).index(i) for i in list(adult) if i != target]]
        ...
        for i in list(adult):
            if adult.loc[:, i].dtype in (int, float) and i != target:
                features2_scale.append(i)
            elif i != target:
                features2_encode.append(i)
        ...
        print('ROD original: ' + str(rod_scores))
        ...
        results.append(
            ['Adult', 'Original', admissible_features + sensitive_features, fold, *rod_scores,
        ...
        dropped_train_df = adult.iloc[
            train_index, [list(adult).index(i) for i in list(adult) if i != target and i != inadmissible_feature and i not in sensitive_features]]
        dropped_test_df = adult.iloc[
            test_index, [list(adult).index(i) for i in list(adult) if i != target and i != inadmissible_feature and i not in sensitive_features]]
        ...
        print('ROD dropped: ' + str(rod_scores))
        ...
        results.append(
            ['Adult', 'dropped', admissible_features, fold, *rod_scores,
    ...
    #                                       inadmissible=inadmissible_feature,
    ...
    #
    #     results.append(
    #         ['Adult', 'NSGAII', selected_names_NSGAII, fold, rod_NSGAII,
    ...
    #
    #     results.append(
    #         ['Adult', 'Kamiran-massaging', list(adult), fold, rod_kamiran,
    ...
    #
    #     X_train = train_df.loc[:, [i for i in list(adult) if i != target]]
    ...
    #
    #     results.append(
    #         ['Adult', 'Kamiran-reweighting ', list(adult), fold, rod_kamiran,
    #          dp_kamiran, tpb_kamiran, tnb_kamiran,
    #          cdp_kamiran,
    #          ctpb_kamiran, ctnb_kamiran, f1_kamiran, end_time])
    #
    ...
    #     X_train_repaired = train_repaired.loc[:, [i for i in list(adult) if i != target]]
    #
    #     X_test_capuchin = adult.iloc[
    #         test_index, [idx for idx, i in enumerate(list(adult)) if i != target]]
    #     X_test_capuchin.reset_index(inplace=True, drop=True)
    #     y_test_capuchin = adult.iloc[test_index, [idx for idx, i in enumerate(list(adult))]]
    ...
    #                            sensitive=sensitive_feature,
    #                            admissible=admissible_features, protected=protected_capuchin, mb=mb)
    #
    ...
    #
    #     results.append(
    #         ['Adult', 'Capuchin ad-hoc', list(adult), fold, rod_capuchin,
    ...
    #     rod_capuchin = ROD.ROD(y_pred=predicted_capuchin_proba, df=X_test_capuchin,
    #                            sensitive=sensitive_feature,
    #                            admissible=admissible_features_capuchin, protected=protected_capuchin, mb=mb)
    #
    #     dp_capuchin = demographic_parity_difference(y_test_capuchin, predicted_capuchin,
    #                                                 sensitive_features=X_test_capuchin.loc[:, sensitive_feature])
    #     tpr_capuchin = MetricFrame(true_positive_rate, y_test_capuchin, predicted_capuchin,
    ...
    #
    #     results.append(
    #         ['Adult', 'Capuchin', all_features_capuchin, fold, rod_capuchin,
    #          dp_capuchin, tpb_capuchin, tnb_capuchin,
    #          cdp_capuchin,
    #          ctpb_capuchin, ctnb_capuchin, f1_capuchin, end_time])
    #
    ...
    results_df = pd.DataFrame(results,
                              columns=['Dataset', 'Method', 'Representation', 'Fold', 'ROD_sex', 'ROD_race', 'DP', 'TPB', 'TNB',
