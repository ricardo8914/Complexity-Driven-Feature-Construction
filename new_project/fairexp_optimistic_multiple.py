...
def run_evaluation(x_train, y_train, df_train, sensitive_feature, sensitive_features, protected, current_names, admissible, clf, parallelism=5):
    ...
    sensitive_protected = list(zip(sensitive_features, protected))
    ...
    for s, p in sensitive_protected:
        rod_train.append(ROD.ROD(y_pred=y_pred_proba_train, df=df_train, sensitive=s,
                            admissible=admissible, protected=p, test_idx=test_indices, mb=mb))
...
def repair_algorithm(train, names, df_train, y_train, sensitive_feature, sensitive_features, protected, admissible_features,
            ...
            if current_representation_train.shape[1] > 1:
                ...
                if len(candidates_f) > 0:
                    ...
                    if max_acc_floating >= global_acc_score:
                        ...
                        global_fair_score = statistics.mean(floating_evaluation[max_index][1])
    ...
    for f in sorted_candidates:
        ...
        if current_representation_train.shape[1] > len(deleted_idx):
            ...
            if statistics.mean(temp_fair_score) >= global_fair_score:
                global_fair_score = statistics.mean(temp_fair_score)
                ...
                if len(deleted_idx) > 0:
                    ...
                    if len(candidates_b) > 0:
                        ...
                        fair_floating_scores = [statistics.mean(y[1]) for y in floating_evaluation]
    ...
    scores = np.asarray([[f[2], f[3], f[4]] for f in registered_representations_train])
    scores = scores.reshape((len(registered_representations_train), 3))
    ...
    selected_representation = registered_representations_train[min_dist][0]
    ...
    train_representation_summary = pd.DataFrame(registered_representations_train,
                                                columns=['Representation', 'Size', 'F1', 'ROD_sex', 'ROD_race', 'CIT', 'Phase'])

    train_representation_summary.to_csv(
        path_or_buf=results_path + '/registered_representations_' + str(dataset_name) + '_' + '_' + str(sampling) + '_' + str(
            fold) + '.csv',
        index=False)
